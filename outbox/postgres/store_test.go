...
func TestConfigRejectsUnsafeIdentifiers(t *testing.T) {
	config := obpostgres.DefaultConfig()
	config.Table = `records; DROP TABLE users`
	if _, err := obpostgres.NewStore(nopDB{}, config); !errors.Is(err, outbox.ErrInvalidArgument) {
		t.Fatalf("NewStore error = %v, want ErrInvalidArgument", err)
	}
}

func TestMigrationContainsFocusedIndexes(t *testing.T) {
	store, err := obpostgres.NewStore(nopDB{}, obpostgres.DefaultConfig())
	if err != nil {
		t.Fatal(err)
	}
...
func TestForFeatureProvidesPostgresStoreService(t *testing.T) {
	dependencies := gioc.NewModule("OutboxPostgresStoreServiceTestDependencies").Provide(
		obpostgres.ProvideConfig(obpostgres.DefaultConfig()),
		gioc.ValueProvider(
	...
	store, err := gioc.Get[*obpostgres.StoreService](container, outbox.OutboxStoreToken, outboxModule)
	if err != nil {
