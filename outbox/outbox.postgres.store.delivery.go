...
func (store *PostgresStore) Claim(ctx context.Context, request ClaimRequest) ([]Record, error) {
	...
	for index := range tokens {
		...
		if tokenErr = ValidateLeaseToken(token, store.config.Limits); tokenErr != nil {
			return nil, tokenErr
		}
...
func (store *PostgresStore) validateClaim(request ClaimRequest) error {
...
func (store *PostgresStore) recoverExpired(ctx context.Context, tx pgx.Tx, now time.Time, limit int) error {
...
func (store *PostgresStore) Acknowledge(ctx context.Context, lease LeaseRef, _ DeliveryResult) error {
	...
	if state == StateDelivered && owner != nil && token != nil && version != nil && *owner == lease.Owner && *token == lease.Token && *version == lease.Version {
		return nil
	}
