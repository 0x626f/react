...
import (
	...
	outboxpostgres "github.com/0x626f/react/outbox/postgres"
...
func Example_postgresStandalone() {
	...
	defer pool.Close()
	config := outboxpostgres.DefaultConfig()
	config.Namespace = "orders"
	store, err := outboxpostgres.NewStore(pool, config)
	if err != nil {
		return
	}
...
func Example_postgresDomainTransaction() {
	...
	defer pool.Close()
	store, err := outboxpostgres.NewStore(pool, outboxpostgres.DefaultConfig())
	if err != nil {
		return
	}
...
func Example_redisStandalone() {
	...
	config.Namespace = "orders"
	config.DurabilityMode = outboxredis.DurabilityRequireAOF
...
func Example_redisDomainComposition() {
	...
	_, _ = store.Compose(context.Background(), outboxredis.CompositionRequest{
